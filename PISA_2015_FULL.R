...
## ---------------------------------------------------------------------------------------------------------------------
resettest(pscores.model7) #Elvetjük H0-t, kell bele nem lineáris hatás, rosszul specifikált a modell
...
train(CNT~.-PV1MATH-PV1SCIE-PV1READ-MISCED-FISCED-BMMJ_Mother-BFMJ_Father-ESCS-ICT_Home-WLE_Cultural-WLE_Home-WLE_Wealth+SocioecStat+ParentalEduc+Vagyon+pmax(WLE_INSTRUCTION-0.7, 0)-Nem-TO-ICT_School-WLE_SchoolWellBeing-ParentalEduc
, data=PISA2015, method="glm", trControl=keresztval)


## ---------------------------------------------------------------------------------------------------------------------
library(precrec)
precrec_obj <- evalmod(scores = becsles, labels = PISA2015$CNT)
...
library(MatchIt)
#match <- matchit(CNT ~ K_200_500 + OV_Koz  + +TS_Koz + TS_Kiv + OED_Koz + OED_Sok + WeeklyClass + ICT_School + WLE_INSTRUCTION+ WLE_TeacherFairness + WLE_Emotional + CPSVALUE + PERFEED + MINS_Maths + MINS_Language + Atlagos + Vagyon, method="nearest", data=PISA2015)
match <- matchit(CNT~.-PV1MATH-PV1READ-PV1SCIE, data = PISA2015, distance=PropScore, method = "full")
