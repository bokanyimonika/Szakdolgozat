...
library(caTools)
set.seed(101) 
sample = sample.split(PISA2018$CNT, SplitRatio = .7)
train = subset(PISA2018, sample == TRUE)
test  = subset(PISA2018, sample == FALSE)
...
train(CNT~.-PV1MATH-PV1SCIE-PV1READ-MISCED-FISCED-BMMJ_Mother-BFMJ_Father-ESCS-ICT_Home-WLE_Cultural-WLE_Home-WLE_Emotional-WLE_Wealth+SocioecStat+ParentalEduc-SzT-PERFEED-WLE_Competitiveness-PERCOOP, data=PISA2018, method="glm", trControl=keresztval)


## ---------------------------------------------------------------------------------------------------------------------
library(precrec)
precrec_obj <- evalmod(scores = becsles, labels = PISA2018$CNT)
...
library(MatchIt)
match <- matchit(CNT~.-PV1MATH-PV1READ-PV1SCIE-PScores-PERCOOP, data = PISA2018, distance=PropScores, method = "full")
